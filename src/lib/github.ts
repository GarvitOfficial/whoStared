...
export const getStargazers = async (owner: string, repo: string, page: number = 1, perPage: number = 30): Promise<GithubUser[]> => {
  const response = await axios.get(`${BASE_URL}/repos/${owner}/${repo}/stargazers`, {
    params: {
      page,
      per_page: perPage,
    },
...
export const getForks = async (owner: string, repo: string, page: number = 1, perPage: number = 30): Promise<GithubUser[]> => {
  const response = await axios.get(`${BASE_URL}/repos/${owner}/${repo}/forks`, {
    params: {
      page,
      per_page: perPage,
    },
  });
  // Forks endpoint returns repo objects, we need the owner of the fork
  return response.data.map((fork: any) => fork.owner);
};
